...
from donate import router as donate_router, handle_pre_checkout, handle_successful_payment, vip_clan_hourly_rating
...
from company import router as company_router, check_company_debts
...
@dp.message(F.text.lower().in_([
    "авто", "машина", "/авто", "/машина", "гараж", "/гараж",
    "активы", "/активы", "актив", "/актив",
    "бизнес", "/бизнес", "биз", "/биз", "бизы", "/бизы",
    "дом", "/дом", "дома", "/дома", "жилье", "/жилье", "жильё", "/жильё",
    "магазин", "/магазин", "шоп", "/шоп", "shop", "/shop",
    "холдинг", "/холдинг",
]))
async def show_assets(message: Message):
    from assets import assets_main_text, assets_main_kb
    await message.answer(assets_main_text(message.from_user.id), parse_mode="HTML", reply_markup=assets_main_kb())
...
@dp.callback_query(F.data == "rzv_assets")
async def cb_rzv_assets(callback: CallbackQuery):
    from assets import assets_main_text, assets_main_kb
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        scheduler.add_job(check_company_debts, "interval", minutes=2, args=[bot])
        scheduler.start()
