...
def _info_text(uid: int, u: dict) -> str:
    ...
    vip_badge = " ⭐ VIP" if don.get("vip") else ""
    ...
    try:
        ...
        farm_name = FARM_LEVELS[farm_lvl]["name"] if farm_lvl > 0 and farm_lvl in FARM_LEVELS else "Нет"
    except Exception:
        btc_bal = 0.0; btc_usd = 0; farm_lvl = 0; farm_name = "Нет"

    # ── Вклад (депозиты) ──
    deposits     = u.get("user_deposits", [])
    deposit_sum  = sum(d.get("amount", 0) for d in deposits if isinstance(d, dict))
    ...
    reg_date = _dt.datetime.fromtimestamp(reg_ts).strftime("%d.%m.%Y") if reg_ts else "—"

    # ── Активы (единая система) ──
    from assets import CATEGORIES as _AST_CATS, total_income_per_hour as _ast_income
    _user_assets = u.get("assets", {})

    def _ast_owned(cat_id):
        for iid, idata in _AST_CATS[cat_id]["items"].items():
            if iid in _user_assets:
                lvl = _user_assets[iid].get("level", 1)
                return f"{idata['name']} (ур.{lvl})"
        return "Нет"

    ast_estate    = _ast_owned("estate")
    ast_business  = _ast_owned("business")
    ast_transport = _ast_owned("transport")
    ast_income_h  = _ast_income(uid)

    # ── СК (строительная компания) ──
    try:
        from company import get_company, OFFICES, _total_emp as _co_emp
        _co = get_company(uid)
        if _co:
            _co_name      = _co.get("name", "Компания")
            _co_lvl       = _co.get("office_level", 1)
            _co_max       = OFFICES[_co_lvl]["max_places"]
            _co_cnt       = _co_emp(_co)
            co_line = f"🏭 СК: <b>{_co_name}</b>  👷 {_co_cnt}/{_co_max}"
        else:
            co_line = "🏭 СК: <b>Нет</b>"
    except Exception:
        co_line = "🏭 СК: <b>Нет</b>"
    ...
    try:
        ...
        if _clan:
            ...
            if _owner_id == uid:
                ...
                _clan_role = "👥 Участник"
            clan_str = f"<b>{_clan_name}</b> ({_clan_role})"
        else:
            clan_str = "<b>Нет</b>"
    except Exception:
        clan_str = "<b>Нет</b>"
    ...
    # ── Шапка ──
    if _admin_role:
        lines.append(f"{_admin_role}{vip_badge}")
    elif vip_badge:
        lines.append(vip_badge.strip())

    lines += [
        f"👤 <b>{name}</b>  🆔 <code>{gid}</code>",
        f"",
        f"💵 Баланс: <b>{format_amount(bal)}$</b>",
        f"💰 Банк: <b>{format_amount(bank)}$</b>",
        f"💿 Биткоины: <b>{_fmt_btc(btc_bal)}</b>  (~{format_amount(btc_usd)}$)",
        f"💎 DC: <b>{dc}</b>",
    ]
    if deposit_sum > 0:
        lines.append(f"🏦 Вклад: <b>{format_amount(deposit_sum)}$</b>")

    lines += [
        f"",
        f"💼 <b>Активы:</b>",
        f"  🏘 Недвижимость: <b>{ast_estate}</b>",
        f"  🏭 Предприятие: <b>{ast_business}</b>",
        f"  🚗 Транспорт: <b>{ast_transport}</b>",
        f"  💰 Доход: <b>{format_amount(ast_income_h)}$/ч</b>",
        f"  {co_line}",
        f"  🎛 Ферма: <b>{farm_name}</b>",
        f"",
        f"🛡 <b>О профиле:</b>",
        f"  🏰 Клан: {clan_str}",
        f"  📆 Создан: <b>{reg_date}</b>",
        f"  📈 Уровень: <b>{level}</b>  ⚡ {_fmt_xp(exp)}/{_fmt_xp(next_exp)} XP",
